...
package body TJa.Keyboard is
  ...
  begin
    if (Mode /= Current_Echo_Mode) then
      case Mode is
        --TODO change these spawns
        when On  => Spawn("/cygdrive/c/cygwin/bin/stty.exe", Args_Echo, Ok);
        when Off => Spawn("/cygdrive/c/cygwin/bin/stty.exe", Args_No_Echo, Ok);
      end case;
  ...
  begin
    if (Mode /= Current_Buffer_Mode) then
      case Mode is
        when On  => Spawn("/cygdrive/c/cygwin/bin/stty.exe", Args_Buffer, Ok);
        when Off => Spawn("/cygdrive/c/cygwin/bin/stty.exe", Args_No_Buffer, Ok);
      end case;
